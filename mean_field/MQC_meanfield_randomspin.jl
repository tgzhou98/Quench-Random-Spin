...
function main(nsite, n_alpha, beta_h, Nrealization, J, xi_arr, tspan, delta_t, total_dirname)
     ...
     for irand in 1:Nrealization
          ...
          u_data = sol.u
          tarr_all = sol.t 
          t_len = length(tarr_all)
          println("norm diff t_arr $(norm(sol.t - t_arr_fix))")
          Sxyz_t = zeros(t_len, 3)

          t_reverse_step_arr = collect(1:10:t_len)
          for i in t_reverse_step_arr
               for j in 1:nsite
                    # Sxyz_t[i,1] += u_data[i][3*(j-1)+1] / nsite
                    # Sxyz_t[i,2] += u_data[i][3*(j-1)+2] / nsite
                    # Sxyz_t[i,3] += u_data[i][3*(j-1)+3] / nsite
                    xval = u_data[i][3*(j-1)+1] * cos(phi) - u_data[i][3*(j-1)+2] * sin(phi)
                    yval = u_data[i][3*(j-1)+2] * cos(phi) + u_data[i][3*(j-1)+1] * sin(phi)
                    u_data[i][3*(j-1)+1] = xval
                    u_data[i][3*(j-1)+2] = yval
                    # u_data[i][3*(j-1)+3] 
               end
          end

          for tstep in t_reverse_step_arr
               tspannew = (0.0, tarr_all[tstep])
               pnew = zeros(Jij_num+4)
               pnew[1:3] = xi_arr
               # Reverse evolution
               pnew[4:Jij_num+3] = - vec(Jij_mat)
               pnew[Jij_num+4] = nsite

               prob = ODEProblem(parameterized_randomspin!,u0,tspannew,pnew)
               sol = solve(prob,reltol=1e-5,adaptive=false,dt=delta_t)
